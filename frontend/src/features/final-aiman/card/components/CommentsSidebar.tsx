...
export const CommentsSidebar = ({
  ...
  const isFromCurrentUser = (c: any) => {
    ...
    const appUserId = (user as any)?.publicMetadata?.userId || (user as any)?.unsafeMetadata?.userId;
  ...
  const handleSubmit = async () => {
    ...
    const temp: CommentDto = { id: `temp-${Date.now()}` as any, cardId, userId: "temp-user" as any, text: trimmed, createdAt: new Date().toISOString() as any } as any;
    setPending((p) => [temp, ...p]);
    setText("");
    try {
  ...
  const renderAvatar = (c: any) => {
    ...
    const initials = (getDisplayName(c) || "?")
      ...
      .toUpperCase();
    return avatar ? (
      <img src={avatar} alt="avatar" className="size-8 shrink-0 rounded-full object-cover" />
    ) : (
      ...
      <div className="space-y-3 flex-1 px-4 pb-4 pt-2">
        ...
        <div className="space-y-3">
          ...
          {pending.map((c) => (
            <div key={c.id} className="flex gap-3 opacity-80">
              ...
              {user?.imageUrl ? (
                <img src={user.imageUrl} alt="avatar" className="size-8 shrink-0 rounded-full object-cover" />
              ) : (
                <div className="size-8 shrink-0 rounded-full bg-gray-600" />
              )}
              <div className="flex-1">
                <div className="text-xs text-gray-400 mb-1">{user?.fullName || user?.username || "You"}</div>
          ...
          {comments?.map((c) => (
            <div key={c.id} className="flex gap-3">
              {renderAvatar(c)}
              <div className="flex-1">
                <div className="text-xs text-gray-400 mb-1">{getDisplayName(c)}</div>
                <div className="rounded-md border border-gray-600 bg-[#161a1d] p-2 text-sm text-white">
                  {c.text}
                </div>
        ...
        {showDetails && (
          <div className="text-xs text-gray-400">
            {`Comments: ${comments?.length ?? 0} • Checklists: ${checklistsCount} • Attachments: ${attachmentsCount}`}
          </div>
        )}
      </div>
    </>
