...
import {
  useCardChecklists,
  useChecklistItems,
} from "@/features/final-final/card/hooks/useCardQueries";
...
const SingleChecklist = ({
  ...
  const addItemMut = useCreateChecklistItem(
    boardId,
    listId,
    cardId,
    checklist.id
  );
  const toggleItemMut = useToggleChecklistItem(
    boardId,
    listId,
    cardId,
    checklist.id
  ...
  const addItem = async () => {
    ...
    if (!t) return;
    const position =
      (items?.reduce((m, i) => Math.max(m, i.position), 0) ?? 0) + 1000;
