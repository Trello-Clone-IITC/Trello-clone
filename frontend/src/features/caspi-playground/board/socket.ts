...
export const emitCreateList = (
  ...
  name: string,
  position?: number
) => getBoardSocket().emit("list:create", { boardId, name, position });
...
export const emitDeleteList = (boardId: string, listId: string) =>
  getBoardSocket().emit("list:delete", { boardId, listId });
...
export const emitDeleteCard = (
  ...
  cardId: string,
  listId: string
) => getBoardSocket().emit("card:delete", { boardId, cardId, listId });
