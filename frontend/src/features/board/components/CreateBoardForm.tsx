...
import type { BoardDto, CreateBoardInput } from "@ronmordo/contracts";
...
export const CreateBoardForm = ({
  ...
  const form = useForm<CreateBoardInput>({
    defaultValues: {
      ...
      background: backgroundOptions[0].id as BoardDto["background"],
    },
    mode: "onChange",
  });

  const selectedBackground = backgroundOptions.find(
    (bg) => bg.id === form.watch("background")
  )!;
            ...
            style={{
              ...
              backgroundImage:
                selectedBackground.type === "photo"
                  ? `url(${
                      selectedBackground.previewUrl || selectedBackground.url
                    })`
                  : selectedBackground.pattern
                  ? `url(${selectedBackground.pattern})`
                  : undefined,
            }}
          >
            <img
