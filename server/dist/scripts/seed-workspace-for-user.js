...
import { Prisma, BoardRole, WorkspaceRole, BoardVisibility, MemberManageRestrictions, CommentingRestrictions, WorkspaceVisibility, WorkspaceType, MembershipRestrictions, BoardCreationRestrictions, BoardSharing, SlackSharing, Color, } from "@prisma/client";
// Usage:
...
async function main() {
    if (!USER_ID)
        throw new Error("Missing USER_ID (env SEED_USER_ID or argv)");
    ...
    const labels = await prisma.$transaction(labelColors.map((l) => prisma.label.create({
        data: { boardId: board.id, name: l.name, color: l.color },
    })));
    // Create lists
    const listNames = ["Backlog", "In Progress", "Review", "Done"];
    const toDec = (n) => new Prisma.Decimal(n);
    const lists = await Promise.all(listNames.map((name, idx) => prisma.list.create({
        data: {
            boardId: board.id,
            name,
            position: toDec((idx + 1) * 100),
            isArchived: false,
            subscribed: false,
        },
    })));
        ...
        for (let ci = 0; ci < 5; ci++) {
            const card = await prisma.card.create({
                data: {
                    ...
                    description: ci % 2 === 0 ? `Description for ${list.name} task ${ci + 1}` : null,
                    position: toDec((ci + 1) * 100),
                    isArchived: false,
                    createdBy: user.id,
                    startDate: ci === 0 ? new Date() : null,
                    dueDate: ci === 0 ? new Date(Date.now() + 7 * 24 * 3600 * 1000) : null,
                },
            });
