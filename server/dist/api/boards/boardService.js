...
export class BoardService {
    static async createBoard(boardData) {
        const { workspace_id, name, description, background, created_by, allow_covers, show_complete, visibility, member_manage, commenting, } = boardData;
        const result = await query(`INSERT INTO boards (
    ...
    static async getBoardWithMembers(id) {
    ...
    static async getBoardsByWorkspace(workspaceId) {
    ...
    static async getBoardsByUser(userId) {
        const result = await query(`SELECT b.* FROM boards b
    ...
    static async updateBoard(id, updates) {
        ...
        values.push(id);
        const result = await query(`UPDATE boards SET ${fields.join(", ")} WHERE id = $${paramCount} RETURNING *`, values);
        return result.rows[0] || null;
    }
    ...
    static async addBoardMember(boardId, userId, role = "member") {
        const result = await query(`INSERT INTO board_members (board_id, user_id, role, joined_at) 
       VALUES ($1, $2, $3, NOW()) 
       RETURNING *`, [boardId, userId, role]);
        return result.rows[0];
    }
    static async removeBoardMember(boardId, userId) {
        const result = await query("DELETE FROM board_members WHERE board_id = $1 AND user_id = $2 RETURNING *", [boardId, userId]);
        return (result.rowCount ?? 0) > 0;
    }
    static async updateBoardMemberRole(boardId, userId, newRole) {
        const result = await query(`UPDATE board_members SET role = $1 
       WHERE board_id = $2 AND user_id = $3 
       RETURNING *`, [newRole, boardId, userId]);
        return result.rows[0] || null;
    }
    static async getBoardMembers(boardId) {
        const result = await query(`SELECT bm.*, u.full_name, u.username, u.avatar_url
       FROM board_members bm
       JOIN users u ON bm.user_id = u.id
       WHERE bm.board_id = $1
       ORDER BY bm.joined_at ASC`, [boardId]);
        return result.rows;
    }
    // List Management
    static async createList(listData) {
        const { board_id, name, position, is_archived, subscribed } = listData;
        const result = await query(`INSERT INTO lists (board_id, name, position, is_archived, subscribed) 
       VALUES ($1, $2, $3, $4, $5) 
       RETURNING *`, [board_id, name, position, is_archived, subscribed]);
        return result.rows[0];
    }
    static async getBoardLists(boardId) {
        const result = await query("SELECT * FROM lists WHERE board_id = $1 ORDER BY position ASC", [boardId]);
        return result.rows;
    }
    static async updateListPosition(listId, newPosition) {
        const result = await query(`UPDATE lists SET position = $1 WHERE id = $2 RETURNING *`, [newPosition, listId]);
        return result.rows[0] || null;
    }
    static async archiveList(listId, archived = true) {
        const result = await query(`UPDATE lists SET is_archived = $1 WHERE id = $2 RETURNING *`, [archived, listId]);
        return result.rows[0] || null;
    }
    static async searchBoards(searchTerm, limit = 10) {
