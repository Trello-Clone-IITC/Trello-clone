...
import { mapBoardDtoToCreateBoardInput, mapBoardDtoToUpdateBoardInput, } from "./board.mapper.js";
...
const getBoardWithMembers = async (id) => {
    const board = await prisma.board.findUnique({
        where: { id },
        include: {
            boardMembers: {
...
const getBoardActivityLogs = async (boardId) => {
...
const archiveList = async (listId, archived = true) => {
    try {
        const list = await prisma.list.update({
            where: { id: listId },
...
const searchBoards = async (searchTerm, limit = 10) => {
    const boards = await prisma.board.findMany({
        where: {
            OR: [
