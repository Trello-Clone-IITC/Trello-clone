...
const getMe = async (clerkUserId) => {
    ...
    const { emailAddresses, fullName, imageUrl } = await clerkClient.users.getUser(clerkUserId);
    if (!fullName) {
...
const getUserIdByClerkId = async (clerkId) => {
    const user = await prisma.user.findUnique({
        where: {
            clerkId,
        },
...
const getUserIdByRequest = async (req) => {
