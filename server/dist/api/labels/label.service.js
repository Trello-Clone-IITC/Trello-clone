...
//   userId: string;
// }
const createLabel = async (data, boardId, userId) => {
    try {
        ...
        if (data.name) {
            ...
            if (existingLabel) {
                throw new AppError("Label with this name already exists on this board", 409);
            }
        }
...
const getLabelById = async (boardId, labelId) => {
    try {
...
const updateLabel = async (updateData, labelId, userId) => {
    try {
