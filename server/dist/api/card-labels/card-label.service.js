...
const addLabelToCard = async (cardId, labelId, userId) => {
    try {
        ...
        if (label.boardId !== card.list.boardId) {
            throw new AppError("Label does not belong to the same board as the card", 400);
        }
        ...
        // Log activity
        await logActivity(card.list.boardId, cardId, userId, ActivityAction.Labeled, {
            type: "card_label",
            labelId: labelId,
...
const removeLabelFromCard = async (cardId, labelId, userId) => {
    ...
    // Log activity
    await logActivity(card.list.boardId, cardId, userId, ActivityAction.Unlabeled, {
        type: "card_label",
        labelId: labelId,
        labelName: label.name,
        labelColor: label.color,
...
const verifyCardAccess = async (cardId, userId) => {
    try {
...
const verifyLabelExists = async (labelId) => {
    try {
...
const checkLabelAssignedToCard = async (cardId, labelId) => {
    try {
...
const createCardLabel = async (cardId, labelId) => {
    try {
...
const deleteCardLabel = async (cardId, labelId) => {
...
const logActivity = async (boardId, cardId, userId, action, payload) => {
    try {
