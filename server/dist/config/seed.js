...
import { PrismaClient, Prisma, Theme, BoardVisibility, MemberManageRestrictions, CommentingRestrictions, BoardRole, BoardSharing, BoardCreationRestrictions, WorkspaceRole, WorkspaceType, WorkspaceVisibility, MembershipRestrictions, SlackSharing, ActivityAction, } from "@prisma/client";
...
async function main() {
    ...
    for (let i = 0; i < WORKSPACES; i++) {
        ...
        const ws = await prisma.workspace.create({
            data: {
                ...
                workspaceMembershipRestrictions: faker.helpers.arrayElement(Object.values(MembershipRestrictions)),
                ...
                allowGuestSharing: faker.helpers.arrayElement(Object.values(BoardSharing)),
        ...
        for (let b = 0; b < BOARDS_PER_WS; b++) {
            ...
            for (let li = 0; li < LISTS_PER_BOARD; li++) {
                await prisma.list.create({
                    data: {
                        boardId: board.id,
                        name: ["To Do", "Doing", "Done", faker.commerce.productName()][li % 4],
                        position: listPos(li),
                        isArchived: false,
                        subscribed: false,
                    },
                    ...
                    const card = await prisma.card.create({
                        data: {
                            ...
                            description: faker.datatype.boolean()
                                ...
                                : null,
                            dueDate: Math.random() < 0.4 ? faker.date.soon({ days: 45 }) : null,
                            startDate: Math.random() < 0.2 ? faker.date.recent({ days: 20 }) : null,
                            position: cardPos(ci),
                            isArchived: false,
                            createdBy: creator,
                            coverImageUrl: Math.random() < 0.25 ? faker.image.urlPicsumPhotos() : null,
    ...
    for (const u of users) {
        const someBoards = pickSome(boards.map((b) => b.id), 3);
