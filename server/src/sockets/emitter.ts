...
export const emitListCreated = (boardId: string, list: Parameters<BoardServerEvents["list:created"]>[0]) => {
  ioRef?.to(room(boardId)).emit("list:created", list);
};
export const emitListUpdated = (boardId: string, list: Parameters<BoardServerEvents["list:updated"]>[0]) => {
...
export const emitCardCreated = (boardId: string, card: Parameters<BoardServerEvents["card:created"]>[0]) => {
  ioRef?.to(room(boardId)).emit("card:created", card);
};
export const emitCardUpdated = (boardId: string, card: Parameters<BoardServerEvents["card:updated"]>[0]) => {
  ioRef?.to(room(boardId)).emit("card:updated", card);
};
export const emitCardDeleted = (boardId: string, cardId: string, listId: string) => {
  ioRef?.to(room(boardId)).emit("card:deleted", cardId, listId);
};
export const emitCardMoved = (boardId: string, card: Parameters<BoardServerEvents["card:moved"]>[0], fromListId?: string) => {
