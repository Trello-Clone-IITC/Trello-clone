...
const updateChecklist = async (
  checklistId: string,
  updateData: UpdateChecklistInput,
  userId: string
) => {
  ...
  if (
    existingChecklist.card.list &&
    existingChecklist.card.list.board.boardMembers.length === 0
  ) {
  ...
  // Log activity
  if (existingChecklist.card.list) {
    await prisma.activityLog.create({
      data: {
...
// Delete checklist
const deleteChecklist = async (checklistId: string, userId: string) => {
  // Verify user has access to the checklist
  const existingChecklist = await prisma.checklist.findFirst({
    where: { id: checklistId },
    include: {
...
// const createChecklistItem = async (data: CreateChecklistItemInput) => {
...
// // Get checklist items
const getChecklistItems = async (checklistId: string, userId: string) => {
  // Verify user has access to the checklist
  const checklist = await prisma.checklist.findFirst({
    where: { id: checklistId },
    include: {
  ...
  if (
    checklist.card.list &&
    checklist.card.list.board.boardMembers.length === 0
  ) {
    throw new AppError("Access denied", 403);
  }
